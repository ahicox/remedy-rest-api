...
async function testGetAttachment(){
	...
	let data = await api.getAttachment({
		schema:		'ahicox:remedy-rest-api demo:data',
		ticket:		'DATA-0000000527',
		fieldName:	'PictureAttachment'
	}).catch(function(e){
		throw(`\t[fail] getAttachment: ${e.toString()}`);
	});
	console.log(`\t[ok]: got: ${typeof(data)} with length ${data.type}`);
	data.type = 'img/png';
	...
	i.src = url;
	document.getElementById('tableContainer').appendChild(i);
...
async function testQueryAndDelete(){
    ...
    let promiseKeeper = [];
    r1.entries.forEach(function(row, idx){
	if (row.values['Entry ID'] != 'DATA-0000000527'){
        promiseKeeper.push(
            api.deleteTicket({
                schema:    'ahicox:remedy-rest-api demo:data',
